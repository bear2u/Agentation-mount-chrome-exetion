...
- 드래그로 임의의 화면 영역을 선택해 하나의 주석으로 기록
- 요소별 코멘트와 우선순위 기록
...
1. `요소 선택` 상태에서 페이지 요소를 클릭하거나, `영역 선택`을 누른 뒤 드래그합니다. 요소 선택 중에는 `Shift`를 누른 채 드래그해도 영역 선택을 시작할 수 있습니다.
2. 원하는 변경사항과 우선순위를 입력하고 `주석 추가`를 누릅니다.
3. 필요한 요소 또는 영역을 계속 선택합니다.
...
네이티브 `<dialog>`를 포함한 브라우저 top layer 팝업 지원은 `0.1.2`부터 포함됩니다. `0.1.4`부터는 팝업을 닫지 않고도 주석 입력창에 텍스트를 입력할 수 있습니다. 최신 ZIP으로 다시 설치하거나 빌드한 뒤 확장과 대상 페이지를 모두 새로고침하세요.
